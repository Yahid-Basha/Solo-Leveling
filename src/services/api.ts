import { User, Quest, Task, ApiResponse, VerificationResult, DashboardData } from '../types';
...
export const api = {
  ...
  async verifyTaskCompletion(
    ...
    tasks[taskIndex] = {
      ...
      verified: true,
      points,
      proof: typeof proofImage === 'string' ? proofImage : 'image-url-placeholder'
    };
    ...
    if (quest) {
      ...
      const completedTasks = questTasks.filter(t => t.status === 'completed' && t.verified);
      quest.progress = Math.min(100, Math.floor((completedTasks.length / Math.max(questTasks.length, 1)) * 100));
  ...
  async retryVerification(
    ...
    const newPoints = tasks[taskIndex].points ? tasks[taskIndex].points + 15 : 50;
  ...
  async getDashboard(userId: string): Promise<ApiResponse<DashboardData>> {
    ...
    const completedTasks = userTasks.filter(t => t.status === 'completed' && t.verified).length;
